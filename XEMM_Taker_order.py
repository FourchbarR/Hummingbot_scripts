...
class SimpleXEMM(ScriptStrategyBase):
    ...
    def place_buy_order(self, exchange: str, trading_pair: str, amount: Decimal):
        # Use market order for hedging
        buy_order = OrderCandidate(trading_pair=trading_pair, is_maker=False, order_type=OrderType.MARKET, order_side=TradeType.BUY, amount=amount)
        buy_order_adjusted = self.connectors[exchange].budget_checker.adjust_candidate(buy_order, all_or_none=False)
        self.buy(exchange, trading_pair, buy_order_adjusted.amount, buy_order_adjusted.order_type)

    def place_sell_order(self, exchange: str, trading_pair: str, amount: Decimal):
        # Use market order for hedging
        sell_order = OrderCandidate(trading_pair=trading_pair, is_maker=False, order_type=OrderType.MARKET, order_side=TradeType.SELL, amount=amount)
        sell_order_adjusted = self.connectors[exchange].budget_checker.adjust_candidate(sell_order, all_or_none=False)
        self.sell(exchange, trading_pair, sell_order_adjusted.amount, sell_order_adjusted.order_type)
