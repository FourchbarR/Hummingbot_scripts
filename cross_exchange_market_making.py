...
async def hedge_filled_maker_order(self, order_filled_event):
    ...
    if market_pair is not None and order_id not in self._taker_to_maker_order_ids.keys():
        ...
        try:
            taker_order_id = await self.check_and_hedge_orders(market_pair)

            # Enregistrer l'heure de placement de l'ordre
            self._taker_order_timestamps[taker_order_id] = time.time()

            # Programmer la conversion en ordre market si l'ordre n'est pas rempli dans 5 minutes
            asyncio.create_task(self.check_order_status(taker_order_id, market_pair))

        except Exception:
            self.log_with_clock(logging.ERROR, "Unexpected error.", exc_info=True)

#Fonction pour checker si l'ordre sur le taker est filled au bout des 5 minutes ou non ; Si non, mettre ordre market

async def check_order_status(self, order_id: str, market_pair):
    await asyncio.sleep(300)  # Attendre 5 minutes

    if order_id in [order.client_order_id for order in self.get_active_orders(market_pair.taker.exchange)]:
        self.log_with_clock(logging.INFO, f"Order {order_id} not filled in 5 minutes, switching to market order.")

        # Annuler l'ordre limite
        self.cancel(market_pair.taker.exchange, market_pair.taker.trading_pair, order_id)

        # Placer un ordre au marché pour la même quantité
        order_to_replace = self._sb_order_tracker.get_shadow_limit_order(order_id)
        if order_to_replace:
            if order_to_replace.trade_type == TradeType.BUY:
                await self.buy(market_pair.taker.exchange, market_pair.taker.trading_pair, order_to_replace.amount, OrderType.MARKET)
            elif order_to_replace.trade_type == TradeType.SELL:
                await self.sell(market_pair.taker.exchange, market_pair.taker.trading_pair, order_to_replace.amount, OrderType.MARKET)

        # Retirer l'ID de l'ordre du dictionnaire
    ...
    #Ajout chatgpt: Si l'ordre limit sur le taker est toujours ouvert après 5 minutes, cancel le et fait un ordre market + Voir modification dans la fonction place_order
    def schedule_order_check(self, market_info, order_id, is_buy, market_pair):
        # Planifie la vérification de l'ordre après 5 minutes
        asyncio.get_event_loop().call_later(300, self.check_limit_order_status, market_info, order_id, is_buy, market_pair)

    async def check_limit_order_status(self, market_info, order_id, is_buy, market_pair):
        # Vérifie si l'ordre limite est toujours actif après 5 minutes
        order_status = await market_info.market.get_order_status(order_id)
        if order_status == OrderStatus.OPEN:
            self.log_with_clock(logging.INFO, f"Order {order_id} not filled after 5 minutes. Cancelling and placing market order.")
            await self.cancel_order(market_info, order_id)
            remaining_amount = await market_info.market.get_order_remaining_amount(order_id)
            
            # Passez l'ordre marché pour la quantité restante
            if remaining_amount > s_decimal_zero:
                if is_buy:
                    await self.buy_with_specific_market(market_info, remaining_amount, order_type=OrderType.MARKET)
                else:
                    await self.sell_with_specific_market(market_info, remaining_amount, order_type=OrderType.MARKET)
        else:
            self.log_with_clock(logging.INFO, f"Order {order_id} has been filled within 5 minutes.")
    #Fin ajout chatgpt 
    ...
    def place_order(self,
        ...
        if is_maker:
            ...
            # Ajouter un suivi pour vérifier l'ordre après 5 minutes
            self.schedule_order_check(market_info, order_id, is_buy, market_pair)
